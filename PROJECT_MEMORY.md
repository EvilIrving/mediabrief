...
## 剪贴板检测必须确认，关闭入口在弹窗 · 2026-08-13 23:50 · grok

桌面窗口可在应用回到前台时检测新复制的视频站点链接，但每次接入都必须用户确认，不能自动开转写。弹窗三个按钮：不再提醒、忽略、开始转写。设置页不做这项开关；关掉检测只靠「不再提醒」。完整需求在 `docs/clipboard-detect-plan.md`。
