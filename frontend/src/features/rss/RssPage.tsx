...
export function RssPage() {
  ...
  /* Persist + update state in one step. */
  ...
  const effectiveActive = filtered.some((f) => f.id === activeFeedId)
    ? activeFeedId
    : filtered[0]?.id || ''
    ...
    try {
      ...
      const parseWithRetry = async (preset: ImportPreset, tries = 3): Promise<RssFeed> => {
        ...
        for (let attempt = 1; attempt <= tries; attempt++) {
          try {
            return await parseFeed(preset.url, t('request_failed') as string)
          } catch (e) {
            lastError = e
      ...
      for (const preset of pending) {
        try {
          ...
          setImportLabel((t('importing_json_feeds') as (a: number, b: number) => string)(done, total))
        }
      }
  ...
  const createTask = async (feedId: string, entryId: string, action: 'summarize' | 'download') => {
    ...
    try {
      ...
      setHandoff({
        ...
        source: {
          type: 'rss',
          value: entry.link || entry.enclosure_url || '',
    ...
    <div className="list-page">
      <div className="page-topbar">
        ...
        <div className="page-topbar-actions">
          <a className="btn-sm" href="/static/rss_feeds_template.json" download title={t('download_json_template')}>
            <span>{t('download_json_template')}</span>
          </a>
          <button className="btn-sm primary" disabled={!!importLabel} onClick={() => jsonInputRef.current?.click()}>
            <span>{importLabel || t('import_json_feeds')}</span>
          </button>
          <input
            ref={jsonInputRef}
            type="file"
      ...
      <div className="rss-add-row">
        <div className="url-wrap">
          <Icon name="i-rss" className="icon url-icon" />
          <input
            type="url"
            className="url-input"
        ...
        <button className="btn-go" disabled={addBusy} onClick={() => void subscribe()}>
          <span>{addBusy ? t('adding') : t('subscribe')}</span>
        </button>
      </div>

      {summaries.length > 0 && (
        <div className="rss-summary-bar">
          <Icon name="i-chart-simple" />
        ...
        <div className="feed-list split-list">
          {!summaries.length ? (
            <div className="rss-empty">
              <div className="rss-empty-icon"><Icon name="i-satellite-dish" /></div>
              <p>{t('rss_empty')}</p>
            </div>
          ) : !filtered.length ? (
            <div className="rss-empty">
              <div className="rss-empty-icon"><Icon name="i-magnifying-glass" /></div>
              <p>{t('rss_no_match')}</p>
            </div>
          ) : (
            filtered.map((f) => {
              ...
              return (
                <div
                  key={f.id}
                  className={`feed-card${f.id === effectiveActive ? ' active' : ''}`}
                  onClick={() => setActiveFeedId(f.id)}
                >
                  <div className="feed-card-header">
                    <div className="feed-card-main" title={f.title}>
                      <div className="feed-card-title">
                        {f.title}{' '}
                        {f.new_count > 0 && <span className="badge feed-new-badge">{(t('new_count') as (n: number) => string)(f.new_count)}</span>}
                      </div>
                      <div className="feed-card-meta">
                        <span className="feed-card-badge">{String(f.type || 'rss').toUpperCase()}</span>
                        <span>{(t('item_count') as (n: number) => string)(f.entry_count)}</span>
                        <span className="feed-card-updated">{t('updated')} {lastChecked}</span>
                        {f.last_error && (
                          <span className="feed-card-error" title={f.last_error}>
                            <Icon name="i-triangle-exclamation" /> {t('rss_refresh_failed')}
                          </span>
                        )}
                      </div>
                    </div>
                    <div className="feed-card-actions" onClick={(e) => e.stopPropagation()}>
                      <button
                        className={`feed-card-del${f.favorite ? ' favorited' : ''}`}
                        data-action="favorite-feed"
                        title={f.favorite ? (t('unfavorite') as string) : (t('favorite') as string)}
                        onClick={() => void toggleFavorite(f.id)}
                      >
                        {f.favorite ? t('unfavorite') : t('favorite')}
                      </button>
                      <button
                        className="feed-card-del"
                        data-action="refresh-feed"
                        disabled={refreshingId === f.id}
                        onClick={() => void refreshFeed(f.id)}
                      >
                        {refreshingId === f.id ? t('processing') : t('refresh')}
                      </button>
                      {pendingDeleteFeed === f.id ? (
                        <span className="inline-confirm">
                          <span className="inline-confirm-message">{t('confirm_delete_feed')}</span>
                          <button className="btn-sm danger" onClick={() => void deleteFeed(f.id)}>{t('delete')}</button>
                          <button className="btn-sm" onClick={() => setPendingDeleteFeed(null)}>{t('cancel')}</button>
                        </span>
                      ) : (
                        <button className="feed-card-del" data-action="delete-feed" onClick={() => setPendingDeleteFeed(f.id)}>
                          {t('delete')}
                        </button>
                      )}
                    </div>
            ...
            <>
              <div className="detail-head">
                ...
                <div className="detail-meta">
                  <span className="feed-card-badge">{String(activeFeed.type || 'rss').toUpperCase()}</span>
                  ...
                  return (
                    <div className="entry-item" key={e.id}>
                      <span className="entry-title" title={e.title}>
                        {isSummarized && <><Icon name="i-file-lines" /> </>}
                        {!isSummarized && isDownloaded && <><Icon name="i-circle-down" /> </>}
                        {e.title}
                      </span>
                      <div className="entry-actions">
                        <button className="btn-sm primary" disabled={creatingTask} onClick={() => void createTask(activeFeed.id, e.id, 'summarize')}>
                          {isSummarized ? t('resummarize') : t('summarize')}
                        </button>
                        {hasAudio && (
                          <button className="btn-sm" disabled={creatingTask} onClick={() => void createTask(activeFeed.id, e.id, 'download')}>
