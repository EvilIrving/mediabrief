...
2. **Config is not `.env`.** Transcribe/summary still take API key / base URL / model from the request when the client sends them. App settings (LLM, bots, TTS, transcription prefs) persist in SQLite via `settings_store.py`. Recovery uses the task's effective LLM config; a bundled `release_config.py` value takes priority in with-Key builds. Do not invent a new env-file config path.
...
6. **Whisper.** Default is `large-v3-turbo` (downloads in the background on first launch). `base` is not bundled by default; `MEDIABRIEF_BUNDLE_BASE_MODEL=1` adds it as an offline fallback, otherwise fallback only uses a model that is already local. Pipeline calls `get_transcriber()` per task, not the frozen `transcriber` singleton. `wait_for_default_model` stays a host latch; do not register it as a Harness Tool.
