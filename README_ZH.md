...
- 🌍 **多语言摘要**: 支持多种语言的智能摘要生成
- 🌐 **多语言界面**: UI 支持 English、中文、日本語、한국어 切换
- 🔧 **自定义AI模型**: 在页面中直接配置任意OpenAI兼容接口（OpenAI、OpenRouter、本地LLM等）——输入API地址和Key，点击 **Fetch** 自动获取可用模型并选择
- ⚙️ **条件式翻译**: 当所选摘要语言与转录语言不一致时，自动生成翻译
- 🗂️ **IndexedDB 历史摘要**: 在 History 标签页直接浏览本地保存的摘要，不必先下载 Markdown 文件
- 🔎 **历史搜索与删除**: 可按标题、内容、来源搜索历史摘要，也可删除不需要的记录
- 📡 **RSS 摘要任务**: 订阅 RSS、刷新条目，并从单条内容创建摘要或下载任务
- ⬇️ **媒体下载中心**: 检测并下载视频、音频、字幕格式
- 📱 **移动适配**: 完美支持移动设备
